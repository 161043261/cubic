...
def gen_cmd_naive(w, a, c, v, ind_path, n_rows, total, ratio, size):
    cmd = './build/nicolas -w {} -a {} -m mix --number-of-queries {} -r {} -c {} -i {} -n {} -v {} --fence-pointer no > dat/{}_{}_{}.dat'.format(
        w, a, int(total * (1 - ratio)), int(total * ratio), c, ind_path, n_rows, v, a,
        'latency' if (v == 'yes') else 'throughput', size)
    return cmd
