...
def gen_cmd_naive(w, a, v, total, ratio, index_path):
    cmd = './build/nicolas -w {} -a {} -m {} --number-of-queries {} -r {} -c {} -i {} -n {} -v {} --fence-pointer no > dat_tmp/figure_{}_{}_raw_w_{}_ratio_{}.dat'.format(
        w, a, mode, int(total * (1 - ratio)), int(total * ratio), cardinality, index_path, number_of_rows, v, a,
        'latency' if (v == 'yes') else 'throughput', w, ratio)
    return cmd


def gen_cmd_cubit(w, a, v, total, ratio, index_path):
    cmd = './build/nicolas -w {} -a {} -m {} --number-of-queries {} -r {} -c {} -i {} -n {} -v {} --fence-pointer yes --fence-length 100000 --merge-threshold {} --rows-per-seg {} --num-embarr-parallel {} --parallel-cnt {} > dat_tmp/figure_{}_{}_raw_w_{}_ratio_{}.dat'.format(
        w, a, mode, int(total * (1 - ratio)), int(total * ratio), cardinality, index_path, number_of_rows, v,
        MERGE_THRESHOLD_CUBIT, ROWS_PER_SEG, NUM_EMBARR_PARALLEL, PARALLEL_CNT, a,
        'latency' if (v == 'yes') else 'throughput', w, ratio)
    return cmd


def gen_cmd_others(w, a, v, total, ratio, index_path):
    cmd = './build/nicolas -w {} -a {} -m {} --number-of-queries {} -r {} -c {} -i {} -n {} -v {} --fence-pointer yes --fence-length 100000 > dat_tmp/figure_{}_{}_raw_w_{}_ratio_{}.dat'.format(
        w, a, mode, int(total * (1 - ratio)), int(total * ratio), cardinality, index_path, number_of_rows, v, a,
        'latency' if (v == 'yes') else 'throughput', w, ratio)
    return cmd
...
def gen_cmd_naive_cardinality(w, a, v, total, ratio, index_path, c, n):
    cmd = './build/nicolas -w {} -a {} -m {} --number-of-queries {} -r {} -c {} -i {} -n {} -v {} --fence-pointer no > dat_tmp/figure_{}_{}_raw_w_{}_ratio_{}_c_{}.dat'.format(
        w, a, mode, int(total * (1 - ratio)), int(total * ratio), c, index_path, n, v, a,
        'latency' if (v == 'yes') else 'throughput', w, ratio, c)
    return cmd


def gen_cmd_cubit_cardinality(w, a, v, total, ratio, index_path, c, n):
    cmd = './build/nicolas -w {} -a {} -m {} --number-of-queries {} -r {} -c {} -i {} -n {} -v {} --fence-pointer yes --fence-length 100000 --merge-threshold {} --rows-per-seg {} --num-embarr-parallel {} --parallel-cnt {} > dat_tmp/figure_{}_{}_raw_w_{}_ratio_{}_c_{}.dat'.format(
        w, a, mode, int(total * (1 - ratio)), int(total * ratio), c, index_path, n, v, MERGE_THRESHOLD_CUBIT,
        ROWS_PER_SEG, NUM_EMBARR_PARALLEL, PARALLEL_CNT, a, 'latency' if (v == 'yes') else 'throughput', w, ratio, c)
    return cmd


def gen_cmd_others_cardinality(w, a, v, total, ratio, index_path, c, n):
    cmd = './build/nicolas -w {} -a {} -m {} --number-of-queries {} -r {} -c {} -i {} -n {} -v {} --fence-pointer yes --fence-length 100000 > dat_tmp/figure_{}_{}_raw_w_{}_ratio_{}_c_{}.dat'.format(
        w, a, mode, int(total * (1 - ratio)), int(total * ratio), c, index_path, n, v, a,
        'latency' if (v == 'yes') else 'throughput', w, ratio, c)
    return cmd
...
def gen_figure_naive_throughput_cardinality():
    ...
    for num in cardinality_number:
        ...
        cmd = gen_cmd_naive_cardinality(core_for_cardinality, 'naive', 'no', 1000, ratio_percentage, index_path, num,
                                        ONE_HUNDRED_MILLION)
        print(cmd)
        os.system(cmd)
        ret = throughput_analysis(
            'dat_tmp/figure_naive_throughput_raw_w_{}_ratio_{}_c_{}.dat'.format(core_for_cardinality, ratio_percentage,
                                                                                num))
        print(ret)
        print('\n')
        f.write('{} {} \n'.format(num, ret))
...
def gen_figure_naive_latency_cardinality():
    ...
    for num in cardinality_number:
        index_path = "100M_{}".format(num)
        cmd = gen_cmd_naive_cardinality(core_for_cardinality, 'naive', 'yes', 1000, ratio_percentage, index_path, num,
                                        ONE_HUNDRED_MILLION)
        print(cmd)
        os.system(cmd)
        ret = latency_analysis(
            'dat_tmp/figure_naive_latency_raw_w_{}_ratio_{}_c_{}.dat'.format(core_for_cardinality, ratio_percentage,
                                                                             num))
        print(ret)
        print('\n')
        for i in range(0, len(ret)):
...
def gen_figure_ucb_throughput_cardinality():
    ...
    for num in cardinality_number:
        index_path = "100M_{}".format(num)
        cmd = gen_cmd_others_cardinality(core_for_cardinality, 'ucb', 'no', 1000, ratio_percentage, index_path, num,
                                         ONE_HUNDRED_MILLION)
        print(cmd)
        os.system(cmd)
        ret = throughput_analysis(
            'dat_tmp/figure_ucb_throughput_raw_w_{}_ratio_{}_c_{}.dat'.format(core_for_cardinality, ratio_percentage,
                                                                              num))
        print(ret)
        print('\n')
        f.write('{} {} \n'.format(num, ret))
...
def gen_figure_ucb_latency_cardinality():
    ...
    for num in cardinality_number:
        index_path = "100M_{}".format(num)
        cmd = gen_cmd_others_cardinality(core_for_cardinality, 'ucb', 'yes', 1000, ratio_percentage, index_path, num,
                                         ONE_HUNDRED_MILLION)
        print(cmd)
        os.system(cmd)
        ret = latency_analysis(
            'dat_tmp/figure_ucb_latency_raw_w_{}_ratio_{}_c_{}.dat'.format(core_for_cardinality, ratio_percentage, num))
        print(ret)
        print('\n')
        for i in range(0, len(ret)):
            f.write('{} {} \n'.format(num, ret[i]))
...
def gen_figure_ub_throughput_cardinality():
    ...
    for num in cardinality_number:
        index_path = "100M_{}".format(num)
        cmd = gen_cmd_others_cardinality(core_for_cardinality, 'ub', 'no', 1000, ratio_percentage, index_path, num,
                                         ONE_HUNDRED_MILLION)
        print(cmd)
        os.system(cmd)
        ret = throughput_analysis(
            'dat_tmp/figure_ub_throughput_raw_w_{}_ratio_{}_c_{}.dat'.format(core_for_cardinality, ratio_percentage,
                                                                             num))
        print(ret)
        print('\n')
        f.write('{} {} \n'.format(num, ret))
...
def gen_figure_ub_latency_cardinality():
    ...
    for num in cardinality_number:
        index_path = "100M_{}".format(num)
        cmd = gen_cmd_others_cardinality(core_for_cardinality, 'ub', 'yes', 1000, ratio_percentage, index_path, num,
                                         ONE_HUNDRED_MILLION)
        print(cmd)
        os.system(cmd)
        ret = latency_analysis(
            'dat_tmp/figure_ub_latency_raw_w_{}_ratio_{}_c_{}.dat'.format(core_for_cardinality, ratio_percentage, num))
        print(ret)
        print('\n')
        for i in range(0, len(ret)):
            f.write('{} {} \n'.format(num, ret[i]))
...
def gen_figure_cubit_lk_throughput_core():
    ...
    for num in core_number:
        ...
        os.system(cmd)
        ret = throughput_analysis(
            'dat_tmp/figure_cubit-lk_throughput_raw_w_{}_ratio_{}.dat'.format(num, ratio_percentage))
        print(ret)
        print('\n')
        f.write('{} {} \n'.format(num, ret))
    f.close()
...
def gen_figure_cubit_lk_throughput_ratio():
    ...
    for num in ratio_number:
        ...
        print(cmd)
        os.system(cmd)
        ret = throughput_analysis('dat_tmp/figure_cubit-lk_throughput_raw_w_{}_ratio_{}.dat'.format(16, num))
        print(ret)
        print('\n')
        f.write('{} {} \n'.format(num, ret))
    f.close()
...
def gen_figure_cubit_lk_throughput_cardinality():
    ...
    for num in cardinality_number:
        index_path = "100M_{}".format(num)
        cmd = gen_cmd_cubit_cardinality(core_for_cardinality, 'cubit-lk', 'no', 1000, ratio_percentage, index_path, num,
                                        ONE_HUNDRED_MILLION)
        print(cmd)
        os.system(cmd)
        ret = throughput_analysis(
            'dat_tmp/figure_cubit-lk_throughput_raw_w_{}_ratio_{}_c_{}.dat'.format(core_for_cardinality,
                                                                                   ratio_percentage, num))
        print(ret)
        print('\n')
        f.write('{} {} \n'.format(num, ret))
    f.close()
...
def gen_figure_cubit_lk_latency_core():  # latency
    ...
    for num in core_number:
        ...
        print(cmd)
        os.system(cmd)
        ret = latency_analysis('dat_tmp/figure_cubit-lk_latency_raw_w_{}_ratio_{}.dat'.format(num, ratio_percentage))
        print(ret)
        print('\n')
        for i in range(0, len(ret)):
            f.write('{} {} \n'.format(num, ret[i]))
...
def gen_figure_cubit_lk_latency_ratio():
    ...
    for num in ratio_number:
        ...
        print(cmd)
        os.system(cmd)
        ret = latency_analysis('dat_tmp/figure_cubit-lk_latency_raw_w_{}_ratio_{}.dat'.format(16, num))
        print(ret)
        print('\n')
        for i in range(0, len(ret)):
            f.write('{} {} \n'.format(num, ret[i]))
...
def gen_figure_cubit_lk_latency_cardinality():
    ...
    for num in cardinality_number:
        index_path = "100M_{}".format(num)
        cmd = gen_cmd_cubit_cardinality(core_for_cardinality, 'cubit-lk', 'yes', 1000, ratio_percentage, index_path,
                                        num, ONE_HUNDRED_MILLION)
        print(cmd)
        os.system(cmd)
        ret = latency_analysis(
            'dat_tmp/figure_cubit-lk_latency_raw_w_{}_ratio_{}_c_{}.dat'.format(core_for_cardinality, ratio_percentage,
                                                                                num))
        print(ret)
        print('\n')
        for i in range(0, len(ret)):
...
def gen_figure_cubit_lf_throughput_core():
    ...
    for num in core_number:
        ...
        os.system(cmd)
        ret = throughput_analysis(
            'dat_tmp/figure_cubit-lf_throughput_raw_w_{}_ratio_{}.dat'.format(num, ratio_percentage))
        print(ret)
        print('\n')
        f.write('{} {} \n'.format(num, ret))
    f.close()
...
def gen_figure_cubit_lf_throughput_ratio():
    ...
    for num in ratio_number:
        ...
        print(cmd)
        os.system(cmd)
        ret = throughput_analysis('dat_tmp/figure_cubit-lf_throughput_raw_w_{}_ratio_{}.dat'.format(16, num))
        print(ret)
        print('\n')
        f.write('{} {} \n'.format(num, ret))
    f.close()
...
def gen_figure_cubit_lf_throughput_cardinality():
    ...
    for num in cardinality_number:
        index_path = "100M_{}".format(num)
        cmd = gen_cmd_cubit_cardinality(core_for_cardinality, 'cubit-lf', 'no', 1000, ratio_percentage, index_path, num,
                                        ONE_HUNDRED_MILLION)
        print(cmd)
        os.system(cmd)
        ret = throughput_analysis(
            'dat_tmp/figure_cubit-lf_throughput_raw_w_{}_ratio_{}_c_{}.dat'.format(core_for_cardinality,
                                                                                   ratio_percentage, num))
        print(ret)
        print('\n')
        f.write('{} {} \n'.format(num, ret))
...
def gen_figure_cubit_lf_latency_core():  # latency
    ...
    for num in core_number:
        ...
        print(cmd)
        os.system(cmd)
        ret = latency_analysis('dat_tmp/figure_cubit-lf_latency_raw_w_{}_ratio_{}.dat'.format(num, ratio_percentage))
        print(ret)
        print('\n')
        for i in range(0, len(ret)):
            f.write('{} {} \n'.format(num, ret[i]))
...
def gen_figure_cubit_lf_latency_ratio():
    ...
    for num in ratio_number:
        ...
        print(cmd)
        os.system(cmd)
        ret = latency_analysis('dat_tmp/figure_cubit-lf_latency_raw_w_{}_ratio_{}.dat'.format(16, num))
        print(ret)
        print('\n')
        for i in range(0, len(ret)):
            f.write('{} {} \n'.format(num, ret[i]))
...
def gen_figure_cubit_lf_latency_cardinality():
    ...
    for num in cardinality_number:
        index_path = "100M_{}".format(num)
        cmd = gen_cmd_cubit_cardinality(core_for_cardinality, 'cubit-lf', 'yes', 1000, ratio_percentage, index_path,
                                        num, ONE_HUNDRED_MILLION)
        print(cmd)
        os.system(cmd)
        ret = latency_analysis(
            'dat_tmp/figure_cubit-lf_latency_raw_w_{}_ratio_{}_c_{}.dat'.format(core_for_cardinality, ratio_percentage,
                                                                                num))
        print(ret)
        print('\n')
        for i in range(0, len(ret)):
...
def cdf():
    ...
    cmd2 = "./build/nicolas -w 16 -a cubit-lk -m mix --number-of-queries 970 -r 30 -c 32 -i 100M_32/ -n 100000000 -v yes --fence-pointer yes --fence-length 100000 > dat_tmp/figure_cubit-lk_latency_raw_w_32_ratio_0.03_c_32.dat"
    cmd3 = "./build/nicolas -w 16 -a cubit-lf -m mix --number-of-queries 970 -r 30 -c 32 -i 100M_32/ -n 100000000 -v yes --fence-pointer yes --fence-length 100000 > dat_tmp/figure_cubit-lf_latency_raw_w_32_ratio_0.03_c_32.dat"
