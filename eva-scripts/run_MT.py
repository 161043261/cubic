...
ratio_percentage = 0.2

core_for_MT = 16

# merge_threshold_num = [1, 2, 4, 8, 16, 32, 64, 128] 
...
def gen_cmd_cubit_MT(w, a, v, total, ratio, index_path, MT):
    cmd = './build/nicolas -p {} -w {} -a {} -m {} --number-of-queries {} -r {} -c {} -i {} -n {} -v {} --fence-pointer yes --fence-length 100000 --rows-per-seg {} --num-embarr-parallel {} --parallel-cnt {} > dat_tmp_MT/figure_{}_{}_raw_w_{}_ratio_{}_MT_{}.dat'.format(
        MT, w, a, mode, int(total * (1 - ratio)), int(total * ratio), cardinality, index_path, number_of_rows, v,
        ROWS_PER_SEG, NUM_EMBARR_PARALLEL, PARALLEL_CNT, a, 'latency' if (v == 'yes') else 'throughput', w, ratio, MT)
    return cmd
...
def gen_figure_cubit_lf_latency_MT():
    ...
    for num in merge_threshold_num:
        ...
        print(cmd)
        os.system(cmd)
        res = latency_analysis(
            'dat_tmp_MT/figure_cubit-lk_latency_raw_w_{}_ratio_{}_MT_{}.dat'.format(core_for_MT, ratio_percentage, num))
        print(res)
        print('\n')
...
def main():
    itr = 0
    cmd = 'mv graphs_MT graphs_MT_{}'
    cmd2 = "mv dat graphs_MT"
    cmd4 = "mv dat_tmp_MT graphs_MT"
    while itr < 2:
        os.chdir(ROOT_PATH)
        ...
        dat_tmp_MT = 'dat_tmp_MT'
        if os.path.exists(dat_tmp_MT) and os.path.isdir(dat_tmp_MT):
            print('Deleting existing directory ./dat_tmp_MT')
            shutil.rmtree(dat_tmp_MT)
        os.mkdir(dat_tmp_MT)

        graphs = 'graphs_MT'
        if os.path.exists(graphs) and os.path.isdir(graphs):
            print('Deleting existing directory ./graphs_MT')
