...
def gen_cmd_cubit_Para(w, a, v, total, ratio, index_path, parallel):
    cmd = './build/nicolas -w {} -a {} -m {} --number-of-queries {} -r {} -c {} -i {} -n {} -v {} --fence-pointer yes --fence-length 100000 --rows-per-seg {} --num-embarr-parallel {} --parallel-cnt {} > dat_tmp_Para/figure_{}_{}_raw_w_{}_ratio_{}_Para_{}.dat'.format(
        w, a, mode, int(total * (1 - ratio)), int(total * ratio), cardinality, index_path, number_of_rows, v,
        ROWS_PER_SEG, parallel, PARALLEL_CNT, a, 'latency' if (v == 'yes') else 'throughput', w, ratio, parallel)
    return cmd
...
def gen_figure_cubit_lf_latency_Para():
    ...
    for num in parallel_num:
        ...
        res = latency_analysis(
            'dat_tmp_Para/figure_cubit-lk_latency_raw_w_{}_ratio_{}_Para_{}.dat'.format(CORES, ratio_percentage, num))
        print(res)
        print('\n')
