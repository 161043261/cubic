...
def gen_cmd_cubit_WPMT(w, a, v, total, ratio, index_path, WPMT):
    cmd = './build/nicolas -q {} -w {} -a {} -m {} --number-of-queries {} -r {} -c {} -i {} -n {} -v {} --fence-pointer yes --fence-length 100000 --merge-threshold {} --rows-per-seg {} --num-embarr-parallel {} --parallel-cnt {} > dat_tmp_WPMT/figure_{}_{}_raw_w_{}_ratio_{}_WPMT_{}.dat'.format(
        WPMT, w, a, mode, int(total * (1 - ratio)), int(total * ratio), cardinality, index_path, number_of_rows, v,
        MERGE_THRESHOLD_NBUB, ROWS_PER_SEG, NUM_EMBARR_PARALLEL, PARALLEL_CNT, a,
        'latency' if (v == 'yes') else 'throughput', w, ratio, WPMT)
    return cmd
...
def gen_figure_cubit_lf_latency_WPMT():
    ...
    for num in workers_num:
        ...
        res = latency_analysis(
            'dat_tmp_WPMT/figure_cubit-lf_latency_raw_w_{}_ratio_{}_WPMT_{}.dat'.format(core_for_WPMT, ratio_percentage,
                                                                                        num))
        print(res)
        print('\n')
