...
def gen_cmd_cubit_Seg(w, a, v, total, ratio, index_path, segs):
    if segs == 1:
        cmd = './build/nicolas -w {} -a {} -m {} --number-of-queries {} -r {} -c {} -i {} -n {} -v {} --fence-pointer yes --fence-length 100000 --rows-per-seg {} --num-embarr-parallel {} --parallel-cnt {} > dat_tmp_Seg/figure_{}_{}_raw_w_{}_ratio_{}_Segs_{}.dat'.format(
            w, a, mode, int(total * (1 - ratio)), int(total * ratio), cardinality, index_path, number_of_rows, v,
            int(number_of_rows * 0.95), NUM_EMBARR_PARALLEL, PARALLEL_CNT, a,
            'latency' if (v == 'yes') else 'throughput', w, ratio, segs)
        return cmd
    else:
        cmd = './build/nicolas -w {} -a {} -m {} --number-of-queries {} -r {} -c {} -i {} -n {} -v {} --fence-pointer yes --fence-length 100000 --rows-per-seg {} --num-embarr-parallel {} --parallel-cnt {} > dat_tmp_Seg/figure_{}_{}_raw_w_{}_ratio_{}_Segs_{}.dat'.format(
            w, a, mode, int(total * (1 - ratio)), int(total * ratio), cardinality, index_path, number_of_rows, v,
            int(number_of_rows / segs), NUM_EMBARR_PARALLEL, PARALLEL_CNT, a,
            'latency' if (v == 'yes') else 'throughput', w, ratio, segs)
        return cmd
...
def gen_figure_cubit_lf_latency_Seg():
    ...
    for num in seg_num:
        ...
        res = latency_analysis(
            'dat_tmp_Seg/figure_cubit-lk_latency_raw_w_{}_ratio_{}_Segs_{}.dat'.format(core_for_Seg, ratio_percentage,
                                                                                       num))
        print(res)
        print('\n')
