...
bind = map(str,
           [0, 4, 8, 12, 16, 20, 24, 28, 1, 5, 9, 13, 17, 21, 25, 29, 2, 6, 10, 14, 18, 22, 26, 30, 3, 7, 11, 15, 19,
            23, 27, 31] + range(33, 64))
...
for nThread in [2, 4, 8, 16, 32, 64]:
    ...
    for pUpdate in [0.5]:
        for appr in ['ub', 'naive', 'ucb']:
            ...
            if appr == 'ub':
                curt = ub_cmd.format(curt_bind, appr, int(10000 * (1 - pUpdate)), int(10000 * pUpdate), nThread - 1,
                                     appr, pUpdate, nThread)
            elif appr == 'ucb':
                curt = ucb_cmd.format(curt_bind, appr, int(10000 * (1 - pUpdate)), int(10000 * pUpdate), nThread - 1,
                                      appr, pUpdate, nThread)
            else:
                curt = naive_cmd.format(curt_bind, appr, int(10000 * (1 - pUpdate)), int(10000 * pUpdate), nThread - 1,
                                        appr, pUpdate, nThread)
            print(curt)
            try:
                os.system(curt)
