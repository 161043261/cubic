...
TransDesc *Cubit::get_rubs_on_btv(uint64_t tsp_begin, uint64_t tsp_end,
    ...
    while (__atomic_load_n(&trans, MM_ACQUIRE) && (READ_ONCE(trans->l_commit_ts) <= tsp_end)) {
        if (READ_ONCE(trans->l_commit_ts) > tsp_begin) {   // The normal case
        ...
        } else if (READ_ONCE(trans->l_commit_ts) == tsp_begin) { ; // Skip the first trans
        } else {
...
TransDesc *Cubit::get_rub_on_row(uint64_t tsp_begin, uint64_t tsp_end,
    ...
    while (__atomic_load_n(&trans, MM_ACQUIRE) && (READ_ONCE(trans->l_commit_ts) <= tsp_end)) {
        ...
        } else if (trans->l_commit_ts == tsp_begin) { ; // Skip the first trans
        } else {
            assert(false);
        }
...
int Cubit::evaluate_Curve(int tid, uint32_t val) {
    ...
    if (tsp_begin_curve < tsp_end_curve) {
        get_rubs_on_btv(tsp_begin_curve, tsp_end_curve, READ_ONCE(bitmap_old_curve->l_start_trans), val_curve,
                        rubs_curve);
    }
    ...
    if (tsp_begin_anchor < tsp_end_anchor) {
        get_rubs_on_btv(tsp_begin_anchor, tsp_end_anchor, READ_ONCE(bitmap_old_anchor->l_start_trans), val_anchor,
                        rubs_anchor);
    }
...
int Cubit::evaluate_common(int tid, uint32_t val) {
    ...
    if ((rubs.size() < merge_threshold) || !reuse_bitmap) {   // will not merge
        if (config->decode) {
            std::vector<uint32_t> dummy;
            if (config->segmented_btv)
