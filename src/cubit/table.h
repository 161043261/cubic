...
#define INV_ROW_ID (numeric_limits<uint64_t>::max())
    ...
    public:
        ...
        virtual int trans_commit(int, uint64_t db_timestamp_t = UINT64_MAX, uint64_t db_row_nums = UINT64_MAX) {
            assert(0);
            return -1;
        };

        virtual int merge_bitmap(int, uint32_t, TransDesc *, Bitmap *, Bitmap *, std::map<uint64_t, RUB> *) {
            assert(0);
            return -1;
        };
