...
po::variables_map get_options(const int argc, const char *argv[]) {
    ...
    desc.add_options()
            ...
            ("workers_per_merge_th,q", po::value<uint32_t>()->default_value(4),
             "number of threads performing merge operation")
            ("merge-threshold,p", po::value<uint32_t>()->default_value(128), "merge threshold for both UpBit and CUBIT")
            ("num-embarr-parallel", po::value<unsigned int>()->default_value(8),
             "num of threads for parallelized get_value/bitmap_init/segbtv_cnt")
