...
void Table::appendBitmap(int to_val) {
    ...
    if (TBC - number_of_rows < buffer[1] * 31) {
        ...
        ibis::bitvector::word_t literal[] = {
                (1U << (number_of_rows % 31)) | buffer[0] | (1U << ibis::bitvector::MAXBITS)};
        fseek(file, size - 9, SEEK_SET);
...
void Table::fastAppend(int to_val) {
    ...
    if (TBC - number_of_rows < buffer[1] * 31) {
        ibis::bitvector::word_t fill0 =
                ((number_of_rows - (TBC - buffer[1] * 31)) / 31) | (1U << ibis::bitvector::MAXBITS);
