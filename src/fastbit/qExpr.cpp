...
ibis::math::term *ibis::math::stdFunction1::reduce() {
    ...
    if (lhs->termType() == ibis::math::NUMBER) {
        ...
        switch (ftype) {
            case ACOS:
                ret = new ibis::math::number(acos(arg));
                break;
            case ASIN:
                ret = new ibis::math::number(asin(arg));
                break;
            case ATAN:
                ret = new ibis::math::number(atan(arg));
                break;
            case CEIL:
                ret = new ibis::math::number(ceil(arg));
                break;
            case COS:
                ret = new ibis::math::number(cos(arg));
                break;
            case COSH:
                ret = new ibis::math::number(cosh(arg));
                break;
            case EXP:
                ret = new ibis::math::number(exp(arg));
                break;
            case FABS:
                ret = new ibis::math::number(fabs(arg));
                break;
            case FLOOR:
                ret = new ibis::math::number(floor(arg));
                break;
            case IS_ZERO:
                ret = new ibis::math::number((arg == 0));
                break;
            case IS_NONZERO:
                ret = new ibis::math::number((0 != arg));
                break;
            case FREXP: {
                int expptr;
                ret = new ibis::math::number(frexp(arg, &expptr));
                break;
            }
            case LOG10:
                ret = new ibis::math::number(log10(arg));
                break;
            case LOG:
                ret = new ibis::math::number(log(arg));
                break;
            case MODF: {
                double intptr;
                ret = new ibis::math::number(modf(arg, &intptr));
                break;
            }
            case ROUND:
                ret = new ibis::math::number(floor(arg + 0.5));
                break;
            case SIN:
                ret = new ibis::math::number(sin(arg));
                break;
            case SINH:
                ret = new ibis::math::number(sinh(arg));
                break;
            case SQRT:
                ret = new ibis::math::number(sqrt(arg));
                break;
            case TAN:
                ret = new ibis::math::number(tan(arg));
                break;
            case TANH:
                ret = new ibis::math::number(tanh(arg));
                break;
            default:
                break;
        }
...
double ibis::math::stdFunction1::eval() const {
    ...
    switch (ftype) {
        ...
        case ibis::math::FABS:
            arg = fabs(arg);
            break;
        case ibis::math::FLOOR:
            arg = floor(arg);
            break;
        case ibis::math::IS_ZERO:
            arg = (double) (0 == arg);
            break;
        case ibis::math::IS_NONZERO:
            arg = (double) (0 != arg);
            break;
        case ibis::math::FREXP: {
            int expptr;
            arg = frexp(arg, &expptr);
            break;
        }
        case ibis::math::LOG10:
            arg = log10(arg);
            break;
        case ibis::math::LOG:
            arg = log(arg);
...
double ibis::math::stdFunction2::eval() const {
    ...
    switch (ftype) {
        case ibis::math::ATAN2:
            lhs = atan2(lhs, rhs);
            break;
        case ibis::math::FMOD:
            lhs = fmod(lhs, rhs);
            break;
        case ibis::math::LDEXP:
            lhs = ldexp(lhs, static_cast<int>(rhs));
            break;
        case ibis::math::POW:
            lhs = pow(lhs, rhs);
            break;
