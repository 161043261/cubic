...
void ibis::bitvector::setBit(const ibis::bitvector::word_t ind, int val, Table_config *config) {
    // m_vec.nosharing(); // make sure the array is not shared
    if (ind >= size()) {
...
/// accesses!
int ibis::bitvector::getBit(const ibis::bitvector::word_t ind, Table_config *config) const {
    if (ind >= size()) {
        return 0;
    } else if (ind >= nbits) {
