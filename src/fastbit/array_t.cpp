...
//
#include "fastbit/array_t.h"
#include "fastbit/util.h"
#include "fastbit/table.h"    // ibis::TYPE_T
...
void ibis::array_t<T>::nosharing() {
    if (m_begin != 0 && m_end >= m_begin) { // a well-formed object
        if (actual == 0 || m_begin != (T *) actual->begin() ||
            ...
            std::cout << "=== WARNING: fastbit is performing copying in nosharing(). MUST CHECK!!! ===" << std::endl;
...
template<class T>
typename ibis::array_t<T>::iterator
...
ibis::array_t<T>::insert(typename ibis::array_t<T>::iterator p,
    ...
    if (m_begin == 0 || m_end < m_begin) { // no space
        ...
        const difference_type nold = ((long int) m_begin > 0 && m_end > m_begin ?
                                      m_end - m_begin : 0);
...
template
class FASTBIT_CXX_DLLSPEC ibis::array_t<char>;

template
class FASTBIT_CXX_DLLSPEC ibis::array_t<signed char>;

template
class FASTBIT_CXX_DLLSPEC ibis::array_t<unsigned char>;

template
class FASTBIT_CXX_DLLSPEC ibis::array_t<float>;

template
class FASTBIT_CXX_DLLSPEC ibis::array_t<double>;

template
class FASTBIT_CXX_DLLSPEC ibis::array_t<int16_t>;

template
class FASTBIT_CXX_DLLSPEC ibis::array_t<int32_t>;

template
class FASTBIT_CXX_DLLSPEC ibis::array_t<int64_t>;

template
class FASTBIT_CXX_DLLSPEC ibis::array_t<uint16_t>;

template
class FASTBIT_CXX_DLLSPEC ibis::array_t<uint32_t>;

template
class FASTBIT_CXX_DLLSPEC ibis::array_t<uint64_t>;

template
class FASTBIT_CXX_DLLSPEC ibis::array_t<ibis::rid_t>;

template
class FASTBIT_CXX_DLLSPEC ibis::array_t<ibis::TYPE_T>;

template
class FASTBIT_CXX_DLLSPEC ibis::array_t<void *>;

template
class FASTBIT_CXX_DLLSPEC ibis::array_t<char *>;

template
class FASTBIT_CXX_DLLSPEC ibis::array_t<const char *>;

template
class FASTBIT_CXX_DLLSPEC ibis::array_t<ibis::bitvector *>;

template
class FASTBIT_CXX_DLLSPEC ibis::array_t<ibis::bitvector const *>;

template
class FASTBIT_CXX_DLLSPEC ibis::array_t<ibis::array_t<ibis::rid_t> *>;
