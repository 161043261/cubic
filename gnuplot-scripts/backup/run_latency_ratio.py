...
def gen_cmd_naive(w, a, v, total, ratio):
    cmd = './build/nicolas -w {} -a {} -m mix --number-of-queries {} -r {} -c 100 -i 100M_100/ -n 100000000 -v {} --fence-pointer no > figure_{}_{}_raw_w_{}_ratio_{}.dat'.format(
        w, a, int(total * (1 - ratio)), int(total * ratio), v, a, 'latency' if (v == 'yes') else 'throughput', w, ratio)
    return cmd


def gen_cmd_others(w, a, v, total, ratio):
    cmd = './build/nicolas -w {} -a {} -m mix --number-of-queries {} -r {} -c 100 -i 100M_100/ -n 100000000 -v {} --fence-pointer yes --fence-length 100000 > figure_{}_{}_raw_w_{}_ratio_{}.dat'.format(
        w, a, int(total * (1 - ratio)), int(total * ratio), v, a, 'latency' if (v == 'yes') else 'throughput', w, ratio)
    return cmd
